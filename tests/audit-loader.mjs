...
const result=await loadGameContent({version:'0.18.0-test'});
if(result.status.mode!=='external'||result.content.cases.length!==6)throw new Error('Carregamento externo válido falhou.');
malformed=true;
const recovered=await loadGameContent({version:'0.18.0-corrupt'});
...
const safe=await loadGameContent({version:'0.18.0-safe',safeMode:true});
