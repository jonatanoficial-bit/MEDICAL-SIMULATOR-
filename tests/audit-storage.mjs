...
localStorage.setItem('medsim-vale-save-v014',JSON.stringify(legacy));
const store=createStateStore({key:'medsim-vale-save-v015',legacyKeys:['medsim-vale-save-v014','medsim-vale-save-v013'],schemaVersion:6,buildVersion:'0.15.0',maxBackups:5,backupIntervalMs:0});
let state=store.load(()=>createDefaultState({buildVersion:'0.15.0'}));
if(state.player.name!=='Dr. Save Antigo'||state.player.xp!==777||state.meta.saveSchema!==6)throw new Error('Migração do save antigo falhou.');
...
const migrated=JSON.parse(localStorage.getItem('medsim-vale-save-v015'));
if(migrated.kind!=='vale-medical-save'||!migrated.checksum||migrated.payload.meta.buildVersion!=='0.15.0')throw new Error('Envelope migrado inválido.');
state.player.xp=900;
...
const stableCurrent=localStorage.getItem('medsim-vale-save-v015');
...
localStorage.setItem(store.pendingKey,JSON.stringify(createEnvelope(interrupted,{kind:'vale-medical-save',schema:6,label:'interrupted-write'})));
localStorage.setItem('medsim-vale-save-v015',stableCurrent);
state=store.load(()=>createDefaultState({buildVersion:'0.15.0'}));
...
const current=JSON.parse(localStorage.getItem('medsim-vale-save-v015'));
current.checksum='00000000';
localStorage.setItem('medsim-vale-save-v015',JSON.stringify(current));
state=store.load(()=>createDefaultState({buildVersion:'0.15.0'}));
