...
const store=createStateStore({key:'medsim-vale-save-v020',legacyKeys:['medsim-vale-save-v017','medsim-vale-save-v015'],schemaVersion:15,buildVersion:'0.24.0',maxBackups:5,backupIntervalMs:0});
let state=store.load(()=>createDefaultState({buildVersion:'0.24.0'}));
if(state.player.name!=='Dr. Save Antigo'||state.player.xp!==777||state.meta.saveSchema!==15)throw new Error('Migração do save antigo falhou.');
...
const migrated=JSON.parse(localStorage.getItem('medsim-vale-save-v020'));
if(migrated.kind!=='vale-medical-save'||!migrated.checksum||migrated.payload.meta.buildVersion!=='0.24.0')throw new Error('Envelope migrado inválido.');
state.player.xp=900;
...
const stableCurrent=localStorage.getItem('medsim-vale-save-v020');
...
localStorage.setItem('medsim-vale-save-v020',stableCurrent);
state=store.load(()=>createDefaultState({buildVersion:'0.24.0'}));
...
const current=JSON.parse(localStorage.getItem('medsim-vale-save-v020'));
current.checksum='00000000';
localStorage.setItem('medsim-vale-save-v020',JSON.stringify(current));
state=store.load(()=>createDefaultState({buildVersion:'0.24.0'}));
