...
const store=createStateStore({key:'medsim-vale-save-v027',legacyKeys:['medsim-vale-save-v026','medsim-vale-save-v017'],schemaVersion:18,buildVersion:'2.0.0',maxBackups:5,backupIntervalMs:0});
let state=store.load(()=>createDefaultState({buildVersion:'2.0.0'}));
...
if(migrated.kind!=='vale-medical-save'||!migrated.checksum||migrated.payload.meta.buildVersion!=='2.0.0')throw new Error('Envelope migrado inválido.');
state.player.xp=900;
...
state=store.load(()=>createDefaultState({buildVersion:'2.0.0'}));
