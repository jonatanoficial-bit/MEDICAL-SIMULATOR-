...
profiles=[
 ('pt-BR','shift-exams','Os resultados têm tempo de processamento',360,800),
 ('en','shift-exams','Results require processing time',360,800),
 ('es','shift-exams','Los resultados requieren tiempo',360,800),
 ('pt-BR','shift-diagnosis','SEGURANÇA MEDICAMENTOSA',360,800),
 ('en','shift-diagnosis','MEDICATION SAFETY',360,800),
 ('es','shift-diagnosis','SEGURIDAD DE MEDICAMENTOS',360,800),
 ('pt-BR','shift-diagnosis','SEGURANÇA MEDICAMENTOSA',768,1024),
 ('en','shift-diagnosis','MEDICATION SAFETY',1366,768),
 ('es','shift-diagnosis','SEGURIDAD DE MEDICAMENTOS',1366,768),
]
results=[]
with sync_playwright() as p:
  ...
  for locale,screen,marker,width,height in profiles:
    ...
    scroll_test=None
    if width==360:
      # Real synthetic touch sequence through CDP to verify vertical scrolling is not cancelled.
      before=page.evaluate('scrollY')
      session=context.new_cdp_session(page)
      session.send('Input.dispatchTouchEvent',{'type':'touchStart','touchPoints':[{'x':180,'y':500,'radiusX':5,'radiusY':5,'force':1}]})
      page.wait_for_timeout(60)
      for y in [470,440,410,380,350,320,290,260,230,200,170,140,110]:
        session.send('Input.dispatchTouchEvent',{'type':'touchMove','touchPoints':[{'x':180,'y':y,'radiusX':5,'radiusY':5,'force':1}]})
        page.wait_for_timeout(25)
      session.send('Input.dispatchTouchEvent',{'type':'touchEnd','touchPoints':[]})
      page.wait_for_timeout(350)
      after=page.evaluate('scrollY')
      scroll_test={'before':before,'after':after,'moved':after>before+20}
    name=f'therapeutics-{locale}-{screen}-{width}x{height}.png'
...
for r in results:
  ...
  if r['scroll'] and r['metrics']['h']>r['viewport'][1]+100 and not r['scroll']['moved']: fail.append(f"{r['locale']}:{r['screen']}: touch scroll")
  if r['pageErrors']: fail.append(f"{r['locale']}:{r['screen']}: errors")
out={'ok':not fail,'method':'Generated production DOM + production CSS + live i18n in isolated Chromium; CDP touch swipe on mobile profiles','failures':fail,'profiles':results}
(ROOT/'docs/audit-therapeutics-layout-v0.24.0.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
