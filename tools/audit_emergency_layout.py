...
profiles=[
 ('pt-BR','pt-BR-emergency-catalog','CENTRO DE EMERGÊNCIA',360,800),
 ('pt-BR','pt-BR-emergency-active','CENTRO DE EMERGÊNCIA',360,800),
 ('en','en-emergency-catalog','EMERGENCY CENTER',360,800),
 ('en','en-emergency-active','EMERGENCY CENTER',1366,768),
 ('es','es-emergency-catalog','CENTRO DE EMERGENCIAS',360,800),
 ('es','es-emergency-active','CENTRO DE EMERGENCIAS',768,1024),
]
results=[]
with sync_playwright() as p:
  ...
  for locale,screen,marker,width,height in profiles:
    ...
    name=f'emergency-{locale}-{screen.split("-")[-1]}-{width}x{height}.png'
...
out={'ok':not fail,'method':'Generated production emergency DOM + production CSS + live i18n in isolated Chromium; CDP touch swipe on mobile profiles','failures':fail,'profiles':results}
(ROOT/'docs/audit-emergency-layout-v0.24.0.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
