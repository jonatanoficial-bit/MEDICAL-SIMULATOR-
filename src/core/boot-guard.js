(function(){
  ...
  const TIMEOUT=12000;
  const ATTEMPT_KEY='medsim-boot-attempts-v027';
  let timer=null;
  let healthy=false;
  ...
  window.VALE_BOOT_GUARD={
    ...
    healthy(){healthy=true;stage='healthy';clearTimeout(timer);try{sessionStorage.setItem(ATTEMPT_KEY,'0');sessionStorage.setItem('medsim-last-healthy-v027',new Date().toISOString());}catch{}},
