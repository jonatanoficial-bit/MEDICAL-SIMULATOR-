...
(function(){
  window.VALE_BUILD_LABEL = 'v0.12.0 | build 2026-06-11 19:01:22 BRT';
  ...
  function ensureBuildBadge(){
    ...
    b.className='build';
    b.textContent='v0.12.0 | build 2026-06-11 19:01:22 BRT';
...
(function(){
  ...
  function addLog(type, detail){
    try {
      var log = readLog();
      log.push({type:type, detail:String(detail||''), at:new Date().toISOString(), build:'v0.12.0 | build 2026-06-11 19:01:22 BRT', width:innerWidth, height:innerHeight});
      writeLog(log);
    } catch(e) {}
  }
  window.ValeDiagnostics = {
    ...
    export:function(){
      try {
        var payload = {build:'v0.12.0 | build 2026-06-11 19:01:22 BRT', diagnostics:readLog()};
        var blob = new Blob([JSON.stringify(payload,null,2)], {type:'application/json'});
        var a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
